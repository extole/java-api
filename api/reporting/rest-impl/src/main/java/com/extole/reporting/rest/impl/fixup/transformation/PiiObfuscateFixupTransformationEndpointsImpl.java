package com.extole.reporting.rest.impl.fixup.transformation;

import javax.ws.rs.ext.Provider;

import org.springframework.beans.factory.annotation.Autowired;

import com.extole.authorization.service.Authorization;
import com.extole.authorization.service.AuthorizationException;
import com.extole.common.rest.exception.FatalRestRuntimeException;
import com.extole.common.rest.exception.RestExceptionBuilder;
import com.extole.common.rest.exception.UserAuthorizationRestException;
import com.extole.common.rest.support.authorization.client.ClientAuthorizationProvider;
import com.extole.id.Id;
import com.extole.reporting.rest.fixup.FixupRestException;
import com.extole.reporting.rest.fixup.transformation.FixupTransformationRestException;
import com.extole.reporting.rest.fixup.transformation.FixupTransformationValidationRestException;
import com.extole.reporting.rest.fixup.transformation.PiiFixupTransformationRequest;
import com.extole.reporting.rest.fixup.transformation.PiiObfuscateFixupTransformationEndpoints;
import com.extole.reporting.rest.fixup.transformation.PiiObfuscateFixupTransformationResponse;
import com.extole.reporting.service.fixup.FixupNotFoundException;
import com.extole.reporting.service.fixup.FixupRuntimeException;
import com.extole.reporting.service.fixup.transformation.FixupTransformationAlreadyExistsException;
import com.extole.reporting.service.fixup.transformation.FixupTransformationNotEditableException;
import com.extole.reporting.service.fixup.transformation.FixupTransformationNotFoundException;
import com.extole.reporting.service.fixup.transformation.FixupTransformationValidationException;
import com.extole.reporting.service.fixup.transformation.PiiObfuscateFixupTransformationBuilder;
import com.extole.reporting.service.fixup.transformation.PiiObfuscateFixupTransformationService;

@Provider
public class PiiObfuscateFixupTransformationEndpointsImpl implements PiiObfuscateFixupTransformationEndpoints {
    private final ClientAuthorizationProvider authorizationProvider;
    private final PiiObfuscateFixupTransformationService piiObfuscateFixupTransformationService;
    private final PiiObfuscateFixupTransformationRestMapper restMapper;

    @Autowired
    public PiiObfuscateFixupTransformationEndpointsImpl(ClientAuthorizationProvider authorizationProvider,
        PiiObfuscateFixupTransformationService piiObfuscateFixupTransformationService,
        PiiObfuscateFixupTransformationRestMapper restMapper) {
        this.authorizationProvider = authorizationProvider;
        this.piiObfuscateFixupTransformationService = piiObfuscateFixupTransformationService;
        this.restMapper = restMapper;
    }

    @Override
    public PiiObfuscateFixupTransformationResponse getTransformation(String accessToken, String fixupId,
        String transformationId)
        throws UserAuthorizationRestException, FixupRestException, FixupTransformationRestException {
        Authorization authorization = authorizationProvider.getClientAuthorization(accessToken);
        try {
            return restMapper.toResponse(piiObfuscateFixupTransformationService.get(authorization, Id.valueOf(fixupId),
                Id.valueOf(transformationId)));
        } catch (AuthorizationException e) {
            throw RestExceptionBuilder.newBuilder(UserAuthorizationRestException.class)
                .withErrorCode(UserAuthorizationRestException.ACCESS_DENIED).withCause(e).build();
        } catch (FixupNotFoundException e) {
            throw RestExceptionBuilder.newBuilder(FixupRestException.class)
                .withErrorCode(FixupRestException.FIXUP_NOT_FOUND)
                .addParameter("fixup_id", e.getFixupId())
                .withCause(e).build();
        } catch (FixupTransformationNotFoundException e) {
            throw RestExceptionBuilder.newBuilder(FixupTransformationRestException.class)
                .withErrorCode(FixupTransformationRestException.TRANSFORMATION_NOT_FOUND)
                .withCause(e)
                .addParameter("fixup_id", e.getFixupId())
                .addParameter("fixup_transformation_id", e.getTransformationId())
                .build();
        }
    }

    @Override
    public PiiObfuscateFixupTransformationResponse createTransformation(String accessToken, String fixupId,
        PiiFixupTransformationRequest request)
        throws UserAuthorizationRestException, FixupRestException, FixupTransformationValidationRestException {
        Authorization authorization = authorizationProvider.getClientAuthorization(accessToken);

        try {
            PiiObfuscateFixupTransformationBuilder transformationBuilder =
                piiObfuscateFixupTransformationService.create(authorization, Id.valueOf(fixupId));
            if (request.getRequestId() != null) {
                transformationBuilder.withRequestId(request.getRequestId());
            }
            return restMapper.toResponse(transformationBuilder.save());
        } catch (AuthorizationException e) {
            throw RestExceptionBuilder.newBuilder(UserAuthorizationRestException.class)
                .withErrorCode(UserAuthorizationRestException.ACCESS_DENIED).withCause(e).build();
        } catch (FixupNotFoundException e) {
            throw RestExceptionBuilder.newBuilder(FixupRestException.class)
                .withErrorCode(FixupRestException.FIXUP_NOT_FOUND)
                .withCause(e)
                .addParameter("fixup_id", e.getFixupId()).build();
        } catch (FixupTransformationAlreadyExistsException e) {
            throw RestExceptionBuilder.newBuilder(FixupTransformationValidationRestException.class)
                .withErrorCode(FixupTransformationValidationRestException.TRANSFORMATION_ALREADY_EXISTS)
                .addParameter("fixup_id", fixupId)
                .withCause(e).build();
        } catch (FixupTransformationValidationException e) {
            throw RestExceptionBuilder.newBuilder(FixupTransformationValidationRestException.class)
                .withErrorCode(FixupTransformationValidationRestException.TRANSFORMATION_VALIDATION)
                .addParameter("fixup_id", fixupId)
                .withCause(e).build();
        }
    }

    @Override
    public PiiObfuscateFixupTransformationResponse updateTransformation(String accessToken, String fixupId,
        String transformationId, PiiFixupTransformationRequest request)
        throws UserAuthorizationRestException, FixupRestException, FixupTransformationRestException {
        Authorization authorization = authorizationProvider.getClientAuthorization(accessToken);

        try {
            PiiObfuscateFixupTransformationBuilder transformationBuilder =
                piiObfuscateFixupTransformationService.update(authorization, Id.valueOf(fixupId),
                    Id.valueOf(transformationId));
            if (request.getRequestId() != null) {
                transformationBuilder.withRequestId(request.getRequestId());
            }
            return restMapper.toResponse(transformationBuilder.save());
        } catch (AuthorizationException e) {
            throw RestExceptionBuilder.newBuilder(UserAuthorizationRestException.class)
                .withErrorCode(UserAuthorizationRestException.ACCESS_DENIED).withCause(e).build();
        } catch (FixupTransformationNotFoundException e) {
            throw RestExceptionBuilder.newBuilder(FixupTransformationRestException.class)
                .withErrorCode(FixupTransformationRestException.TRANSFORMATION_NOT_FOUND)
                .withCause(e)
                .addParameter("fixup_id", e.getFixupId())
                .addParameter("fixup_transformation_id", e.getTransformationId())
                .build();
        } catch (FixupNotFoundException e) {
            throw RestExceptionBuilder.newBuilder(FixupRestException.class)
                .withErrorCode(FixupRestException.FIXUP_NOT_FOUND)
                .withCause(e)
                .addParameter("fixup_id", e.getFixupId())
                .build();
        } catch (FixupTransformationNotEditableException e) {
            throw RestExceptionBuilder.newBuilder(FixupTransformationRestException.class)
                .withErrorCode(FixupTransformationRestException.NOT_EDITABLE)
                .withCause(e).build();
        }
    }

    @Override
    public PiiObfuscateFixupTransformationResponse deleteTransformation(String accessToken, String fixupId,
        String transformationId)
        throws UserAuthorizationRestException, FixupRestException, FixupTransformationRestException {
        Authorization authorization = authorizationProvider.getClientAuthorization(accessToken);
        try {
            return restMapper
                .toResponse(piiObfuscateFixupTransformationService.delete(authorization, Id.valueOf(fixupId),
                    Id.valueOf(transformationId)));
        } catch (AuthorizationException e) {
            throw RestExceptionBuilder.newBuilder(UserAuthorizationRestException.class)
                .withErrorCode(UserAuthorizationRestException.ACCESS_DENIED).withCause(e).build();
        } catch (FixupNotFoundException e) {
            throw RestExceptionBuilder.newBuilder(FixupRestException.class)
                .withErrorCode(FixupRestException.FIXUP_NOT_FOUND)
                .withCause(e)
                .addParameter("fixup_id", e.getFixupId()).build();
        } catch (FixupTransformationNotFoundException e) {
            throw RestExceptionBuilder.newBuilder(FixupTransformationRestException.class)
                .withErrorCode(FixupTransformationRestException.TRANSFORMATION_NOT_FOUND)
                .addParameter("fixup_id", e.getFixupId())
                .addParameter("fixup_transformation_id", e.getTransformationId())
                .withCause(e).build();
        } catch (FixupRuntimeException e) {
            throw RestExceptionBuilder.newBuilder(FatalRestRuntimeException.class)
                .withErrorCode(FatalRestRuntimeException.SOFTWARE_ERROR).withCause(e).build();
        } catch (FixupTransformationNotEditableException e) {
            throw RestExceptionBuilder.newBuilder(FixupTransformationRestException.class)
                .withErrorCode(FixupTransformationRestException.NOT_EDITABLE)
                .withCause(e).build();
        }
    }
}
