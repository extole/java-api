...
public class CampaignControllerActionScheduleResponseMapper implements
    ...
    @Autowired
    public CampaignControllerActionScheduleResponseMapper(CampaignComponentRestMapper campaignComponentRestMapper) {
        this.campaignComponentRestMapper = campaignComponentRestMapper;
    }

    @Override
    public CampaignControllerActionScheduleResponse toResponse(CampaignControllerActionSchedule action,
        ZoneId timeZone) {
    ...
    @Override
    public CampaignControllerActionScheduleConfiguration toConfiguration(CampaignControllerActionSchedule action,
        ZoneId timeZone, Map<Id<CampaignComponent>, String> componentNames) {
        List<ZonedDateTime> dates = action.getScheduleDates().stream()
            .map(date -> date.atZone(timeZone))
            .collect(Collectors.toList());

        return new CampaignControllerActionScheduleConfiguration(Omissible.of(Id.valueOf(action.getId().getValue())),
