...
public class CampaignComponentComponentIdVariableResponse extends CampaignComponentVariableResponse {
    public static final String SETTING_TYPE = "COMPONENT_ID";

    private static final String JSON_FILTER = "filter";

    private final ComponentIdFilterResponse filter;

    @JsonCreator
    public CampaignComponentComponentIdVariableResponse(@JsonProperty(JSON_COMPONENT_SETTING_NAME) String name,
        ...
        @JsonProperty(JSON_FILTER) ComponentIdFilterResponse filter) {
        super(name, displayName, type, values, source, description, tags, priority);
        this.filter = filter;
    }

    @JsonProperty(JSON_FILTER)
    public ComponentIdFilterResponse getFilter() {
        return filter;
    }
