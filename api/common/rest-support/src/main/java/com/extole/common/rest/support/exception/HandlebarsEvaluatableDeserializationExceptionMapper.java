...
@Provider
@Priority(1)
public class HandlebarsEvaluatableDeserializationExceptionMapper
    implements ExceptionMapper<HandlebarsEvaluatableDeserializationException> {
    private static final Logger LOGGER = LoggerFactory.getLogger(
        HandlebarsEvaluatableDeserializationExceptionMapper.class);
    ...
    @Override
    public Response toResponse(HandlebarsEvaluatableDeserializationException exception) {

        EvaluatableRestException restException = RestExceptionBuilder.newBuilder(EvaluatableRestException.class)
            .withErrorCode(EvaluatableRestException.HANDLEBARS_EXPRESSION_INVALID_SYNTAX)
            .withCause(exception)
