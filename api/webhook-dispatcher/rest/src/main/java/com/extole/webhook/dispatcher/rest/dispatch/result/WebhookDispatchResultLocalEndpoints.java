...
public interface WebhookDispatchResultLocalEndpoints {

    @Produces(APPLICATION_JSON)
    @GET
    @Path("/{webhook_id}/dispatch-results/recent")
    @Operation(summary = "View recent webhook dispatch results for the pod",
        description = "Retrieves webhook dispatch results - most recent first - from the pod requested. Best used with"
            + " api-X.extole.io rather than api.extole.io. For general purposes please use the non-local endpoint"
            + " api.extole.io/v6/webhooks/{webhook_id}/dispatch-results/recent")
    List<WebhookDispatchResultResponse> getLocalWebhookDispatchResults(@UserAccessTokenParam String accessToken,
